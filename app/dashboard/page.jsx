...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Add event listener for roadmap step completion
    const handleRoadmapStepCompleted = () => {
      fetchDashboardData();
    };

    window.addEventListener('roadmapStepCompleted', handleRoadmapStepCompleted);

    // Cleanup
    return () => {
      window.removeEventListener('roadmapStepCompleted', handleRoadmapStepCompleted);
    };
  }, [user]);
  ...
  const calculateRoadmapProgress = (roadmaps) => {
    ...
    const totalProgress = roadmaps.reduce((sum, roadmap) => sum + (roadmap.progress || 0), 0);
    return Math.round(totalProgress / roadmaps.length);
  };

  const calculateRoadmapStepProgress = (roadmap) => {
    if (!roadmap.content?.steps) return 0;
